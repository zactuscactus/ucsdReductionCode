...
dssSolution.Solve;
V_o(:,kk)=dssCircuit.AllBusVmagPu;

dssCircuit.Loads.First;
for ii=1:15
	loadOrig=dssCircuit.Loads.kw;
	kvarOrig=dssCircuit.Loads.kvar;
	dssCircuit.Loads.kw=dssCircuit.Loads.kw*((kk-1)/10);
	dssCircuit.Loads.kvar=kvarOrig;
	dssSolution.Solve;
	Vp(ii,:,kk)=dssCircuit.AllBusVmagPu;
	dVp(ii,:,kk)=V_o-Vp(ii,:,kk);
	dP(ii,:,kk)=loadOrig;
	PTDF_p(ii,:,kk)=dVp(ii,:,kk)./dP(ii,:,kk);
