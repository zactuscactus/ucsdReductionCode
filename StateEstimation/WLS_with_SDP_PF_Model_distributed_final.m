function [WLS_SDP_Result]=WLS_with_SDP_PF_Model_distributed_final(Yk,Ykbar,Ykl,Yklbar,M,Z,volt1,volt,D,true_volt,parent,generation)
...
DEL_final=repmat(del,1,length(Buses));
VMAG_final=repmat(Vmag,1,length(Buses));
...
%                 disp('NAN');
            ...
            DEL(:,busId)=del;
            VMAG(:,busId)=Vmag;
            offspring_nodes=[[Nodes2{generation{busId,2}}]';Nodes{busId}];
            DEL_final(Nodes{busId},busId)=del(Nodes{busId});
            VMAG_final(Nodes{busId},busId)=Vmag(Nodes{busId});
            DEL_final(offspring_nodes,parent(busId))=DEL_final(offspring_nodes,busId);
            VMAG_final(offspring_nodes,parent(busId))=VMAG_final(offspring_nodes,busId);
            ...
            Delerr(iter,:,busId)=mod(del-del0+pi,2*pi)-pi;
            offspring_nodes=[[Nodes2{generation{busId,2}}]';Nodes{busId}];
            DEL_final(Nodes{busId},busId)=DEL(Nodes{busId},parent(busId));
            VMAG_final(Nodes{busId},busId)=VMAG(Nodes{busId},parent(busId));
            DEL_final(offspring_nodes,parent(busId))=DEL_final(offspring_nodes,busId);
            VMAG_final(offspring_nodes,parent(busId))=VMAG_final(offspring_nodes,busId);
%             figure(1);
...
%    E = E + dE;
    ...
    if mod(iter,1)==0
        for busId=1:length(Buses)
            ...
            figure(100);
            subplot(2,1,1);plot([Vmag0,VMAG_final(:,end)]); xlabel('Bus Number'); ylabel('Voltage Magnitude (pu)'); legend({'True' 'Estimated' });
            subplot(2,1,2);plot([del0,DEL_final(:,end)]*180/pi); xlabel('Bus Number'); ylabel('Voltage Angle (degree)'); legend({ 'True' 'Estimated'});
        pause(2);
    end
