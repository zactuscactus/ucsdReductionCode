            function [State_Estimate]=Estimate_State_fastdecoupled(ybus,zdata,bpq,dssCircuit,Ycomb,Ybase,true_volt)
            ...
            V00 = ones(n_nodes,1); % Initialize the bus voltages..
            V=V00;
            del00=round(angle(true_volt)/(pi/6))*(pi/6);
            del=del00;
            ...
            %     end
                ...
                for i = 1:npi
                    ...
                    for k = 1:(Free_angles)
                        if k+1 == m
                            for n = 1:n_nodes
                                H21(i,k) = H21(i,k) + V00(m)* V00(n)*(-G(m,n)*sin(del00(m)-del00(n)) + B(m,n)*cos(del00(m)-del00(n)));
                            end
                            H21(i,k) = H21(i,k) - V00(m)^2*B(m,m);
                        else
                            H21(i,k) = V00(m)* V00(k+1)*(G(m,k+1)*sin(del00(m)-del00(k+1)) - B(m,k+1)*cos(del00(m)-del00(k+1)));
                        end
                    end
                end
                ...
                for i = 1:npi
                    ...
                    for k = 1:(n_nodes)
                        if k == m
                            for n = 1:n_nodes
                                H22(i,k) = H22(i,k) + V00(n)*(G(m,n)*cos(del00(m)-del00(n)) + B(m,n)*sin(del00(m)-del00(n)));
                            end
                            H22(i,k) = H22(i,k) + V00(m)*G(m,m);
                        else
                            H22(i,k) = V00(m)*(G(m,k)*cos(del00(m)-del00(k)) + B(m,k)*sin(del00(m)-del00(k)));
                        end
                    end
                end
                ...
                for i = 1:nqi
                    ...
                    for k = 1:(Free_angles)
                        if k+1 == m
                            for n = 1:n_nodes
                                H31(i,k) = H31(i,k) + V00(m)* V00(n)*(G(m,n)*cos(del00(m)-del00(n)) + B(m,n)*sin(del00(m)-del00(n)));
                            end
                            H31(i,k) = H31(i,k) - V00(m)^2*G(m,m);
                        else
                            H31(i,k) = V00(m)* V00(k+1)*(-G(m,k+1)*cos(del00(m)-del00(k+1)) - B(m,k+1)*sin(del00(m)-del00(k+1)));
                        end
                    end
                end
                ...
                for i = 1:nqi
                    ...
                    for k = 1:(n_nodes)
                        if k == m
                            for n = 1:n_nodes
                                H32(i,k) = H32(i,k) + V00(n)*(G(m,n)*sin(del00(m)-del00(n)) - B(m,n)*cos(del00(m)-del00(n)));
                            end
                            H32(i,k) = H32(i,k) - V00(m)*B(m,m);
                        else
                            H32(i,k) = V00(m)*(G(m,k)*sin(del00(m)-del00(k)) - B(m,k)*cos(del00(m)-del00(k)));
                        end
                    end
                end
                        ...
                        if any(ismember(strsplit(AllElements{elem},'.'),'Line')) || any(ismember(strsplit(AllElements{elem},'.'),'Vsource')) || any(ismember(strsplit(AllElements{elem},'.'),'Transformer'))
                            ...
                            i=find(all([fbus(pf),tbus(pf)]'==repmat([Nodeorder1{elem}(node),Nodeorder2{elem}(node)],length(tbus(pf)),1)'));
                                ...
                                for k = 1:(Free_angles)
                                    if k+1 == m
                                        for n = 1:n_nodes
                                            H41(i,k) = H41(i,k) + V00(m)* V00(n)*(-G(m,n)*sin(del00(m)-del00(n)) + B(m,n)*cos(del00(m)-del00(n)));
                                        end
                                        H41(i,k) = H41(i,k) - V00(m)^2*B(m,m);
                                    else
                                        H41(i,k) = V00(m)* V00(k+1)*(G(m,k+1)*sin(del(m)-del00(k+1)) - B(m,k+1)*cos(del00(m)-del(k+1)));
                                    end
                                end
                                for k = 1:(n_nodes)
                                    if k == m
                                        for n = 1:n_nodes
                                            H42(i,k) = H42(i,k) + V00(n)*(G(m,n)*cos(del00(m)-del00(n)) + B(m,n)*sin(del00(m)-del00(n)));
                                        end
                                        H42(i,k) = H42(i,k) + V00(m)*G(m,m);
                                    else
                                        H42(i,k) = V00(m)*(G(m,k)*cos(del00(m)-del00(k)) + B(m,k)*sin(del00(m)-del00(k)));
                                    end
                                end
...
%                                         H41(i,k) = H41(i,k) - V00(m)^2*B(m,m);
%                                     else
%                                         H41(i,k) = V(m)* V00(k+1)*(G(m,k+1)*sin(del00(m)-del00(k+1)) - B(m,k+1)*cos(del00(m)-del00(k+1)));
%                                     end
%                                 end
%                                 for k = 1:(n_nodes)
%                                     if k == m
    ...
    % 
                            ...
                            i=find(all([fbus(qf),tbus(qf)]'==repmat([Nodeorder1{elem}(node),Nodeorder2{elem}(node)],length(tbus(qf)),1)'));
                                ...
                                for k = 1:(Free_angles)
                                    if k+1 == m
                                        for n = 1:n_nodes
                                            H51(i,k) = H51(i,k) + V00(m)* V00(n)*(G(m,n)*cos(del00(m)-del00(n)) + B(m,n)*sin(del00(m)-del00(n)));
                                        end
                                        H51(i,k) = H51(i,k) - V00(m)^2*G(m,m);
                                    else
                                        H51(i,k) = V00(m)* V00(k+1)*(-G(m,k+1)*cos(del00(m)-del00(k+1)) - B(m,k+1)*sin(del00(m)-del00(k+1)));
                                    end
                                end
                                for k = 1:(n_nodes)
                                    if k == m
                                        for n = 1:n_nodes
                                            H52(i,k) = H52(i,k) + V00(n)*(G(m,n)*sin(del00(m)-del00(n)) - B(m,n)*cos(del00(m)-del00(n)));
                                        end
                                        H52(i,k) = H52(i,k) - V00(m)*B(m,m);
                                    else
                                        H52(i,k) = V00(m)*(G(m,k)*sin(del00(m)-del00(k)) - B(m,k)*cos(del00(m)-del00(k)));
                                    end
                                end
