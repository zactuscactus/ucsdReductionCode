...
%% Gen Y bus
	...
	if isfield(circuit,'regcontrol');
		circuit_woPV=rmfield(circuit_woPV,'regcontrol');
    end
%     if isfield(circuit,'capacitor');
% 		circuit_woPV=rmfield(circuit_woPV,'capacitor');
% 	end

	
	
	%load the circuit and generate the YBUS
	p = dsswrite(circuit_woPV,[],0,pwd); o = actxserver('OpendssEngine.dss');
	...
	busnames=regexp(dssCircuit.YNodeOrder,'\.','split');
 	YbusOrderVect=[busnames{:}]'; YbusOrderVect=YbusOrderVect(1:2:end);%YbusOrderVect(find(cellfun('length',YbusOrderVect)==1))=[];
 	YbusPhaseVect=[busnames{:}]'; YbusPhaseVect=YbusPhaseVect(2:2:end);%YbusPhaseVect=(find(cellfun('length',YbusPhaseVect)>1))=[]; YbusPhaseVect=str2double(YbusPhaseVect);
    Ycomb=strcat(YbusOrderVect,'.', (YbusPhaseVect));
...
%             4*ones(Nbranch,1),Sline(2:2:end,1:2),Sline(2:2:end,4),.02*ones(Nbranch,1)];
        ...
        % Low error on Zero-injecton buses and voltage reference buses
%         Z1(find(Z1(:,4)<.001),5)=Z1(find(Z1(:,4)<.001),5)/100;
%         Z2(find(Z2(:,4)<.001),5)=Z2(find(Z2(:,4)<.001),5)/100;
