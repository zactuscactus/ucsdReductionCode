...
for i=1:length(propn)
	...
	switch(lower(propn{i}))
		...
		case {'name','like','bus1','dynadll'}
            val{i} = dataclean(val{i},'name');
		case 'phases'
			val{i} = dataclean(val{i},'phase'); 
		case {'kv','kva','kvar','kwhrated','pf','kwhstored','charge','discharge','effcharge','effdischarge','r','x','model','vminpu','vmaxpu','class','basefreq'}
			val{i} = dataclean(val{i},'num');
        case 'conn'
            val{i} = dataclean(val{i},'conn');
		case {'debugtrace','enabled'}
