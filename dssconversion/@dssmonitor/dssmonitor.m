function obj = dssmonitor(s,varargin)
% OpenDSS monitor object
...
if(nargin==1 && isa(s,cn))
	...
	obj.defaults = struct('Name','', ...
		...
		'Element','',...
		'Terminal',[],...
		'Enabled','',... %{Yes|No or True|False} 
		'PPolar','YES',... %{Yes/True | No/False} Default = YES. Report power in Apparent power, S, in polar form (Mag/Angle).(default)  Otherwise, is P and Q
		'Residual','NO',... %{Yes/True | No/False} Default = No.  Include Residual cbannel (sum of all phases) for voltage and current. Does not apply to sequence quantity modes or power modes.
		'VIPolar','YES',... %{Yes/True | No/False} Default = YES. Report voltage and current in polar form (Mag/Angle). (default)  Otherwise, it will be real and imaginary.
		'Mode',[],...
		'Action','' ...
			%{
				{Clear | Save | Take}
