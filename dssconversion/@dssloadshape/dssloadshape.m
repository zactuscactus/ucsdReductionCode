function obj = dssloadshape(s,varargin)
% OpenDSS load shape object
...
if(nargin==1 && isa(s,cn))
	...
	obj.defaults = struct('Name','', ...
		...
		'Sngfile','', ...
		'Dblfile','', ...
		'Action','', ... % {Normalize | DblSave | SngSave}
		'UseActual','',... % {Yes | No* | True | False*}
		'Pmax',[],... % used for snapshot solution
		'Qmax',[],... % used for snapshot solution
		'Pbase',0 );
