function obj = dssswtcontrol(s,varargin)
% OpenDSS Switch Control object
...
if(nargin==1 && isa(s,cn))
	...
	obj.defaults = struct( ...
        'Name',{''},...
        'Action','Close',... % {Open | Close}  simulates manual operation of the controlled switch to open or close, after a time delay. Note: automatic operation requires use of the COM interface with an external control algorithm.
        'basefreq',60,... 
        'Delay',120,... % Operating time delay (sec) of the switch. Defaults to 120.
        'enabled','Yes',... % {Yes|No or True|False} Indicates whether this element is enabled.
        'Lock',0,... %TODO: test for what kinds/values does this field allows, Controlled switch is locked in its present open / close state. Switch will not respond to either manual (Action) or automatic (COM interface) control until this Lock is removed.
        'SwitchedObj','',... % Name of circuit element switch that the SwtControl operates. Specify the full object class and name.
        'SwitchedTerm',1 ...% Terminal number of the controlled element switch. 1 or 2, typically.  Default is 1.
    );
