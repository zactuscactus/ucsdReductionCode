...
for i=1:length(propn)
	...
	switch(lower(propn{i}))
		...
		case {'kvaemerg','kvanormal','basefreq','mask','peakcurrent','terminal'}
			val{i} = dataclean(val{i},'num');
		case {'threephaselosses','enabled','linelosses','localonly','losses','phasevoltagereport','seqlosses','vbaselosses','xfmrlosses'}
			val{i} = dataclean(val{i},'logical','string');
		case 'conn'
			val{i} = dataclean(val{i},'conn');
		case {'name','like','element'}
			val{i} = dataclean(val{i},'name');
		case {'action','option'}
			% normal string. look at dssenergymeter.m to see their
			% specifications
		case {'zonelist'}
			if ~any(val{i}=='=')
