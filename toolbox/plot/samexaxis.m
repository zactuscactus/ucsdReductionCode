...
%	* spaceRatio (default=.02 or 2%): horizontal space between subplots wrt subplot height
%
%
% Example:
...
%
% Copyright from Aslak Grinsted 2005. Modified by Dung (Andu) Nguyen
% Andu: added feature(s): spaceRatio


Args=struct('YAxisLocation','left','XAxisLocation','bottom','Box','on','XLim',[],'XTick',[],'XTickLabel',[],'XMinorTick',[],'YLabelDistance',1.4,'ABC',false,'Join',false,'YTickAntiClash',false,'spaceRatio',.02); 
...
if Args.Join
    ...
    Hused=sum(pos(:,4));
	% horizonatl space between subplots
	stretch=max(H/Hused,1)*(1-Args.spaceRatio*(length(ax)-1));
    pos(1,2)=pos(1,2)-pos(1,4)*(stretch-1);
    pos(:,4)=pos(:,4)*stretch;
    set(ax(1),'pos',pos(1,:));
    for ii=2:length(ax)
        pos(ii,2)=pos(ii-1,2)-pos(ii,4)-max(H/Hused,1)*Args.spaceRatio;
...
if Args.ABC
    for ii=1:length(ax)
        text(0,.98,[' ' 'a'+ii-1 ')'],'units','normalized','verticalalignment','top','parent',ax(ii));
    end
end
