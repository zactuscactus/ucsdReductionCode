function [ data ] = dssSimulation_TS( circuit, mode, t,V_predict, nodeOrder)
...
while dssSolution.dblHour <= simHours
	...
	NewArr=circuit.xycurve(curveInd).xarray+dv(ii);
	allOneString = sprintf('%.0f,' ,NewArr);
	allOneString = allOneString(1:end-1);% strip final comma
	dssText.command=['XYcurve.' char(curveNames(curveInd)) '.Xarray=(' allOneString ')'];
	end
