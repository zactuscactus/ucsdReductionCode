function [circuit, circuit_orig, powerFlowFull, powerFlowReduced, pathToDss] = reducingFeeders_Final_SI(pathToFile,criticalBuses,savePath,debug)
...
% 				circuit.load(ii).vmaxpu=[];
		...
		if isfield(circuit,'pvsystem')
			for ii=1:length(circuit.pvsystem)
				circuit.pvsystem(ii).kva=circuit.pvsystem(ii).pmpp*1.05;
			end
		end
...
if debug
	...
	if ~exist([savePath feeder '_sim.mat'])  || ~useSaved
		orig=tic;
		[ powerFlowFullControl ] = dssSimulationControl( circuit,[],1,[],[],0);
		[ powerFlowFull ] = dssSimulation1( circuit,[],1,[],[],0);
		FullTime=toc(orig);
		save([savePath feeder '_sim.mat'],'powerFlowFull','powerFlowFullControl')
	else
...
[Flag]=isfield(circuit,{'load','pvsystem','capacitor','transformer','regcontrol','capcontrol','reactor','generator','buslist','invcontrol'});
...
if ~exist([savePath feeder '_Ybus.mat'])  || ~useSaved
	...
	if Flag(10)
		circuitBase=rmfield(circuitBase,'invcontrol');
		circuitBase=rmfield(circuitBase,'xycurve');
	end
	if exist('o')
		delete(o);
		clearvars o
	...
	[ YcombV, vmag,theta,volt_base,voltComplex]=getVolt(circuitBase);
...
if Flag(1)
	...
	if ~exist([savePath feeder '_Load.mat'])  || ~useSaved
		...
		Map_LDo_NODEo=zeros(length(ld),length(Ycomb));
		for j=1:length(ld)
			[~,data] = char(ld(j));
			...
			elseif length(regexp(ld(j).bus1,'\.','match'))>1 %2 PhaseInd
				...
				if numPhases==2 && strcmpi(ld(j).conn,'delta')
					...
					if strcmpi([name{2} '.' name{3}],'1.2')||strcmpi([name{2} '.' name{3}],'2.3')||strcmpi([name{2} '.' name{3}],'3.1')
						...
						Map_LDo_NODEo(j,Ind1)=Ra;
						Map_LDo_NODEo(j,Ind2)=Rb;
...
if Flag(2)
	...
	if ~exist([savePath feeder '_PV.mat'])  || ~useSaved
		...
		kva_PV=zeros(length(YbusOrderVect),1);
		% 	kV_PV=zeros(length(YbusOrderVect),length(YbusOrderVect));
		Map_PVo_NODEo=zeros(length(pv),length(Ycomb));
...
% 			P=pv(j).pf*pv(j).kVA; Q=sqrt(pv(j).kVA^2-P^2);
			...
			if isempty(regexp(pv(j).bus1,'\.','match')) %3 PhaseInd
				...
				kva_PV(Ind)=kva_PV(Ind)+pvKVA/3;
				Map_PVo_NODEo(j,Ind)=1/3;
			...
			elseif length(regexp(pv(j).bus1,'\.','match'))>1 %2 PhaseInd
				...
				if length(name)==3 && strcmpi(pv(j).conn,'delta')
					...
					if strcmpi([name{2} '.' name{3}],'1.2')||strcmpi([name{2} '.' name{3}],'2.3')||strcmpi([name{2} '.' name{3}],'3.1')
						...
						Map_PVo_NODEo(j,Ind1)=Ra*1;
						Map_PVo_NODEo(j,Ind2)=Rb*1;
					elseif strcmpi([name{2} '.' name{3}],'2.1')||strcmpi([name{2} '.' name{3}],'3.2')||strcmpi([name{2} '.' name{3}],'1.3')
						S_PV(Ind1)=S_PV(Ind1)+Rb*pvPower;
						S_PV(Ind2)=S_PV(Ind2)+Ra*pvPower;
						Map_PVo_NODEo(j,Ind1)=Rb*1;
						Map_PVo_NODEo(j,Ind2)=Ra*1;
					end
				else
					for ii=2:length(name)
						Ind=find(ismemberi(Ycomb,[name{1} '.' name{ii}]));
						S_PV(Ind)=S_PV(Ind)+pvPower/pv(j).phases;
						kva_PV(Ind)=kva_PV(Ind)+pvKVA/pv(j).phases;
						Map_PVo_NODEo(j,Ind)=1/pv(j).phases;
					end
				end
			else %1 PhaseInd
				...
				kva_PV(Ind)=kva_PV(Ind)+pvKVA;
				Map_PVo_NODEo(j,Ind)=1;
			end
		end
		...
		save([savePath feeder '_PV.mat'],'S_PV','kva_PV','Map_PVo_NODEo')
	else
...
deltaV=zeros(length(Ycomb_org),length(Ycomb_org));
deltaV_pu=zeros(length(Ycomb_org),length(Ycomb_org));
...
% % % %calcV distflow
% % % distV(find(ismemberi(YbusOrderVect,'sourcebus')))=1;
% % % count=0;
% % % for ii=1:length(generation(:,1))
...
% % % 		vk=distV(Bus1IndMod)
% % % 		rk=-real(inv(Ybus(Bus1IndMod,Bus2Ind)));
% % % 		qk=-imag(inv(Ybus(Bus1IndMod,Bus2Ind)));
% % % 		%Change top include pv/gen
% % % 		pk=sum(S_LD(),2)
% % % 		distV(Bus2Ind)=sqrt(vk.^2-2*(
% % % 	end
% % % end

pv_weighting=W2*diag(S_PV); pv_weighting(find(abs(pv_weighting)<1E-3))=0;
kva_PV_red=abs(W2*diag(kva_PV));
end
tRed=toc;
fprintf('%.2f sec\n',tRed)
...
for ii=1:length(generation(:,1))
	...
	for jj=1:length(Connected)
		...
		if any(volt_base(Bus1Ind(1))>volt_base(Bus2Ind(1))) || any(ismemberi(find(ismemberi(trfBus(:,1),Bus1)),find(ismemberi(trfBus(:,2),Bus2))))
			continue
		else
			Bus1IndMod=Bus1Ind(find(ismemberi(YbusPhaseVect(Bus1Ind),YbusPhaseVect(Bus2Ind))));
...
% % 			circuit.capacitor(end).kv=round(volt_base(BusInds(1))/sqrt(3)*100)/100;
% % 			w=2*pi*60;
...
if ~exist('deltaV','var')
	...
	singleInv=0;
	if length(circuit_orig.invcontrol)==1
		singleInv=1;
		curveInd=find(ismemberi(circuit_orig.xycurve(:).name,circuit_orig.invcontrol.vvc_curve1));
	end
	
	[NegIndsRow,NegIndsCol]=find(real(pv_weighting)<0);
	if ~exist('S_LD_new','var')
		S_LD_new=zeros(length(Ycomb),1);
	end
	S_LD_new(NegIndsRow,1)=S_LD_new(NegIndsRow,1)-sum(pv_weighting(sub2ind(size(pv_weighting),NegIndsRow,NegIndsCol)),2);
% 	pv_weighting([NegIndsRow,NegIndsCol])=0;
	pv_weighting(sub2ind(size(pv_weighting),NegIndsRow,NegIndsCol))=0;
	%create efficiency and p vs t curves (required for invcontrol)
	circuit.xycurve=dssxycurve;
	circuit.xycurve(1).Name='myeff';
	circuit.xycurve(1).npts=4;
	circuit.xycurve(1).xarray=[0 25 75 100];
	circuit.xycurve(1).yarray=[1 1 1 1];
	
	circuit.xycurve(2).Name='mypvst';
	circuit.xycurve(2).npts=4;
	circuit.xycurve(2).xarray=[.1 .2 .4 1];
	circuit.xycurve(2).yarray=[1 1 1 1];
	
	count=0;
	circuit.pvsystem=dsspvsystem;
	[WritePVsRow,WritePVsCol]=find(abs(pv_weighting)>0.1);
% 	Ratio=abs(pv_weighting./repmat(S_PV',12,1));
	Map_PVr_NODEo=zeros(length(WritePVsRow),length(Ycomb_org));
	for ii=1:length(WritePVsRow)
		P=real(pv_weighting(WritePVsRow(ii),WritePVsCol(ii))); P(abs(P)<1E-2)=0;
		Q=imag(pv_weighting(WritePVsRow(ii),WritePVsCol(ii))); Q(abs(Q)<1E-2)=0;
		if P==0 &&Q==0
			continue
		end
		count=count+1;
		...
		circuit.pvsystem(count)=dsspvsystem;
		circuit.pvsystem(count).Name=['PV_' char(Ycomb(WritePVsRow(ii))) '_from_' char(Ycomb_org(WritePVsCol(ii)))];
		circuit.pvsystem(count).phases=1;
		circuit.pvsystem(count).irradiance=1;
		circuit.pvsystem(count).kV= round(volt_base(WritePVsRow(ii))/sqrt(3)*100)/100;
		circuit.pvsystem(count).bus1=Ycomb(WritePVsRow(ii));
		circuit.pvsystem(count).pmpp=P;
		circuit.pvsystem(count).kvar=Q;
		circuit.pvsystem(count).kVA=kva_PV_red(WritePVsRow(ii),WritePVsCol(ii));
		circuit.pvsystem(count).cutout=0;
		circuit.pvsystem(count).cutin=0;
		circuit.pvsystem(count).EffCurve='myeff';
		circuit.pvsystem(count).PTCurve='mypvst';
		circuit.pvsystem(count).vminpu=.01;
		Map_PVr_NODEo(ii,WritePVsCol(ii))=1;%Ratio(WritePVsRow(ii),WritePVsCol(ii));
		
		%do inverter things here
		%eventually code for many different initial inverter controls with
		%different types and curves
		circuit.invcontrol(count)=dssinvcontrol;
		circuit.invcontrol(count).name=['InvForPV_' char(Ycomb(WritePVsRow(ii))) '_from_' char(Ycomb_org(WritePVsCol(ii)))];
		circuit.invcontrol(count).PVSystemList=['PV_' char(Ycomb(WritePVsRow(ii))) '_from_' char(Ycomb_org(WritePVsCol(ii)))];
		circuit.invcontrol(count).Mode=circuit_orig.invcontrol.Mode;
		circuit.invcontrol(count).vvc_curve1=['CurveFor_InvForPV_' char(Ycomb(WritePVsRow(ii))) '_from_' char(Ycomb_org(WritePVsCol(ii)))];
		circuit.invcontrol(count).DeltaQ_factor=.01;
		circuit.xycurve(end+1).Name=['CurveFor_InvForPV_' char(Ycomb(WritePVsRow(ii))) '_from_' char(Ycomb_org(WritePVsCol(ii)))];
		circuit.xycurve(end).npts=6;
		
% 		%find dV
% 		colInd=WritePVsCol(ii);
% 		rowInd=find(ismember(Ycomb_org,Ycomb(WritePVsRow(ii))));
% 		dV=deltaVactual(rowInd,colInd);
% 		circuit.xycurve(end).xarray=circuit_orig.xycurve(curveInd).xarray;%+dV;
% 		circuit.xycurve(end).yarray=circuit_orig.xycurve(curveInd).yarray;
	end
	
	circuit.Map_PVo_NODEo=Map_PVo_NODEo;
	circuit.Map_PVr_NODEo=Map_PVr_NODEo;
...
if exist('S_LD_new','var')
	...
	Map_LDr_NODEo=zeros(1,length(Ycomb_org));
	for loadModel=1:size(S_LD_new,2)
		...
		for ii=1:length(WriteLoads)
			...
			circuit.load(count).vminpu=.01;
			Map_LDr_NODEo(count,WriteLoads(ii))=1;
...
% 			end
	...
	circuit.Map_LDo_NODEo=Map_LDo_NODEo;
	circuit.Map_LDr_NODEo=Map_LDr_NODEo;
	t_=toc;
	fprintf('%.2f sec\n',t_)
end
...
if debug
	...
	%plotting
	redd=tic;
	[ powerFlowReducedControl ] = dssSimulationControl( circuit,[],1,[],[],0);
	[ powerFlowReduced ] = dssSimulation1( circuit,[],1,[],[],0);
	redTime=toc(redd);
	
% 	for ii=1:length(powerFlowReduced.nodeName)
% 		Keep(ii)=find(ismemberi(powerFlowFullControl.nodeName,powerFlowReducedControl.nodeName(ii)));
% 	end
	for ii=1:length(powerFlowReduced.nodeName)
		Keep2(ii)=find(ismemberi(powerFlowFull.nodeName,powerFlowReduced.nodeName(ii)));
	end
% 	voltDiffControl=abs(powerFlowFullControl.Voltage(Keep2)-powerFlowReducedControl.Voltage)'
% 	voltDiff=abs(powerFlowFull.Voltage(Keep2)-powerFlowReduced.Voltage)'
% 	[B,I]=sort(voltDiff);
% 	powerFlowReducedControl.nodeName(I);
% 	[diffV, ind]=max(abs(powerFlowFullControl.Voltage(Keep)-powerFlowReducedControl.Voltage));
% 	[~,sorted ] =sort(powerFlowReducedControl.Dist);
...
% 	for ii=1:length(circuit.load)
% 		circuit.load(ii).name=['load' num2str(ii)];
% 	end
% 	for ii=1:length(circuit.pvsystem)
% 		circuit.pvsystem(ii).name=['PV' num2str(ii)];
% 	end
