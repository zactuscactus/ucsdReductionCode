function [circuit, circuit_orig, powerFlowFull, powerFlowReduced, pathToDss,voltDiff] = reducingFeeders(pathToFile,criticalBuses,savePath,debug)
...
% yShRl=real(Ybus_justShunt); yShRl(abs(yShRl)<1E-6)=0;
% yShIm=imag(Ybus_justShunt); yShIm(abs(yShIm)<1E-6)=0;
...
for ii=1:length(generation(:,1))
	...
	r_jx=Ybus_justShunt(Bus1Ind);
	inv_r_jx=1./r_jx;
	
	for rr=1:length(r_jx)
		if isinf(1./r_jx(rr))
			continue
		end
			...
			circuit.line(end).name=['addedShuntLine_' char(Ycomb(Bus1Ind(rr)))];
			circuit.line(end).phases=1;
			circuit.line(end).bus1=char(Ycomb(Bus1Ind(rr)));
			circuit.line(end).bus2=[char(YbusOrderVect(Bus1Ind(rr))) '.0'];
			circuit.line(end).Rmatrix=real(inv_r_jx(rr));
			circuit.line(end).xmatrix=imag(inv_r_jx(rr));
			circuit.line(end).cmatrix=0;
			circuit.line(end).length=1;

		
% % 		%figure out if it should be reactor or capacitor
% % 		if real(inv_r_jx(rr))<=0 && imag(inv_r_jx(rr))<=0
% % 			
...
% % 			circuit.capacitor(end).name=['addedCapShunt_' char(Ycomb(Bus1Ind(rr)))];
% % 			circuit.capacitor(end).phases=1;
% % 			circuit.capacitor(end).bus1=char(Ycomb(Bus1Ind(rr)));
% % 			circuit.capacitor(end).kv=round((VreducedVectAbs(Bus1Ind(rr))/sqrt(3))*100)/100;
% % 			w=2*pi*60;
% % 			circuit.capacitor(end).R=-real(inv_r_jx(rr));
% % 			circuit.capacitor(end).cmatrix=imag(r_jx(rr))*(1/w)*1E6;
% % 			
% % 		elseif real(inv_r_jx(rr))<=0 && imag(inv_r_jx(rr))>=0
% % 			%fix
% % 			if real(inv_r_jx(rr))~=0
% % 			if ~isfield(circuit,'capacitor')
% % 				circuit.capacitor(1)=dsscapacitor;
% % 			else
% % 				circuit.capacitor(end+1)=dsscapacitor;
...
% % 			w=2*pi*60;
% % 			circuit.capacitor(end).R=-real(inv_r_jx(rr));
% % 			circuit.capacitor(end).cmatrix=0;
% % 			end
% % 			
% % 			if imag(inv_r_jx(rr))~=0
% % 			
...
% % 			circuit.reactor(end).name=['addedReactorShunt_' char(Ycomb(Bus1Ind(rr)))];
% % 			circuit.reactor(end).phases=1;
% % 			circuit.reactor(end).bus1=char(Ycomb(Bus1Ind(rr)));
% % 			circuit.reactor(end).kv=round((VreducedVectAbs(Bus1Ind(rr))/sqrt(3))*100)/100;
% % 			circuit.reactor(end).R=0;
% % 			circuit.reactor(end).X=imag(inv_r_jx(rr));		
% % 			end
% % 		elseif real(inv_r_jx(rr))>=0 && imag(inv_r_jx(rr))<=0
% % 			if imag(inv_r_jx(rr))~=0
% % 			if ~isfield(circuit,'capacitor')
...
% % 			circuit.reactor(end).bus1=char(Ycomb(Bus1Ind(rr)));
% % 			circuit.reactor(end).kv=round((VreducedVectAbs(Bus1Ind(rr))/sqrt(3))*100)/100;
% % 			circuit.reactor(end).R=real(inv_r_jx(rr));
% % 			circuit.reactor(end).X=0;
% % 			end
% % 		elseif real(inv_r_jx(rr))>=0 && imag(inv_r_jx(rr))>=0
% % 			
% % 			if ~isfield(circuit,'reactor')
