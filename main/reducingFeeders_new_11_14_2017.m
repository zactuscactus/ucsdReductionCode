function [circuit, circuit_orig, powerFlowFull, powerFlowReduced, pathToDss,voltDiff] = reducingFeeders_11_14_2017(pathToFile,criticalBuses,savePath,debug)
...
% 	V2=volt_base.*exp(1i*theta');
	...
	save([savePath feeder '_Ybus.mat'],'Ybus','YbusOrg','YbusOrderVect','YbusPhaseVect','Ycomb','buslist','Zbus','circuit_orig','V2','VoltVect','volt_base');
else
...
% yRl=real(Ybus_justShunt); yRl(abs(yRl)<1E-9)=0;
% yIm=imag(Ybus_justShunt); yIm(abs(yIm)<1E-9)=0;
% Ybus_justShunt=yRl+1i*yIm;
...
for ii=1:length(generation(:,1))
	...
	for jj=1:length(Connected)
		...
		if Match
			continue
		elseif any(volt_base(Bus1Ind(1))>volt_base(Bus2Ind(1)))
			...
			if length(Bus1IndMod)==length(Bus2Ind)
				...
				if round(real(Zb(2,1)/zbase*100)*100)/100<0
					stopping=1;
				end
...
if exist('S_PV_new','var')
	...
	for ii=1:length(WritePVs)
		...
		circuit.pvsystem(count).kV= volt_base(WritePVs(ii))/sqrt(3);
...
if exist('S_LD_new','var')
	...
	for loadModel=1:size(S_LD_new,2)
		...
		for ii=1:length(WriteLoads)
			...
			circuit.load(count).kV= volt_base(WriteLoads(ii))/sqrt(3);
