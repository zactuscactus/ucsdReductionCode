...
%updated 5/23/17
%updated 9/20/17
%this code is archived 11/14/2017. 
%It is not workign well for how we are handling capacitors, we need to
%handle capacitors very differently, hwence the new code.
...
yShRl=real(Ybus_reduced); yShRl(abs(yShRl)<1E-9)=0;
yShIm=imag(Ybus_reduced); yShIm(abs(yShIm)<1E-9)=0;
Ybus_reduced=yShRl+1i*yShIm;
...
% 				circuit.transformer(end).xlt=round(imag(Zb(2,1)/zbase*100)*100)/100;
				circuit.transformer(end).rs=[(real(Zb(2,1)/zbase*100)*100)/100 ((real(Zb(1,1)/zbase*100)-real(Zb(2,1)/zbase*100))*100)/100 ((real(Zb(2,2)/zbase*100)-real(Zb(2,1)/zbase*100))*100)/100];
				circuit.transformer(end).xhl=(imag(Zb(1,1)/zbase*100)*100)/100;
				circuit.transformer(end).xht=(imag(Zb(2,2)/zbase*100)*100)/100;
				circuit.transformer(end).xlt=-2*(imag(Zb(2,1)/zbase*100)*100)/100+(imag(Zb(2,2)/zbase*100)*100)/100+(imag(Zb(1,1)/zbase*100)*100)/100;
...
if Flag(1)
	...
	for loadModel=1:size(S_LD_new,2)
		WriteLoads=find(abs(S_LD_new(:,loadModel))>.001);
...
% 			circuit.load(count).KVAr=imag(S_LD_new(WriteLoads(ii),loadModel));
			...
			if sign(P)==sign(Q)
				circuit.load(count).pf=abs(P/sqrt(P^2+Q^2));
			else
				if sign(P)
