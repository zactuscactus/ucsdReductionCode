function [circuit, circuit_orig, powerFlowFull, powerFlowReduced, pathToDss,voltDiff,Meas0,Meas] = reducingFeeders_Final(pathToFile,criticalBuses,savePath,debug,Meas)
...
measFlag=exist('Meas','var');
if measFlag
	%reorganize measurements into vectors to reduce
	Pinj=zeros(length(Ycomb),1);
	Qinj=Pinj;
	V=Pinj;
	Meas0=Meas;
	
	%remove and store source
	srcInd=[find(ismemberi(strtok(Meas(:,2),'\.'),'source'));find(ismemberi(strtok(Meas(:,3),'\.'),'source'))];
	MeasSrc=Meas(srcInd,:);
	Meas(srcInd,:)=[];
	
	indPinj=find([Meas{:,1}]==1);
	indQinj=find([Meas{:,1}]==2);
	indPline=find([Meas{:,1}]==3);
	indQline=find([Meas{:,1}]==4);
	indV=find([Meas{:,1}]==5);
	
	Pinj(getMatchingOrder(Meas(indPinj,2),Ycomb))=[Meas{indPinj,4}];
	Qinj(getMatchingOrder(Meas(indQinj,2),Ycomb))=[Meas{indQinj,4}];
	SigmaPinj(getMatchingOrder(Meas(indPinj,2),Ycomb))=[Meas{indPinj,5}];
	SigmaQinj(getMatchingOrder(Meas(indQinj,2),Ycomb))=[Meas{indQinj,5}];
	V(getMatchingOrder(Meas(indV,2),Ycomb))=[Meas{indV,4}];
	
	%make Pmeas
	Pmeas=zeros(length(Ycomb),length(Ycomb),2);
	
	Pind=sub2ind(size(Pmeas),getMatchingOrder(Meas(indPline,2),Ycomb),getMatchingOrder(Meas(indPline,3),Ycomb),ones(length(indPline),1));
	Qind=sub2ind(size(Pmeas),getMatchingOrder(Meas(indQline,2),Ycomb),getMatchingOrder(Meas(indQline,3),Ycomb),2*ones(length(indPline),1));
	Pmeas(Pind)=([Meas{indPline,4}]+1i*[Meas{indPline,5}]);
	Pmeas(Qind)=([Meas{indQline,4}]+1i*[Meas{indQline,5}]);
	
		%delete line measurements 
	Meas([indPline; indQline],:)=[];
...
if ~exist([savePath feeder '_initTopo.mat'])  || ~useSaved
	...
	clear generation;  generation{1,1}=1; generation{1,4}=[];generation{1,5}=0; generation{1,6}=0; generation{:,7}=zeros(3,2);generation{:,8}=zeros(3,2);
	parent=1;
	topo(parent,1)=parent;
	[topo,generation]=topology_detect_large(topo,generation,Ybus,parent,Node_number,distMat,Pmeas,YbusPhaseVect);
	topo_view=topo;
...
if measFlag
	Sinj_new=W2*(Pinj+1i*Qinj);
	Sigma_new=(W2.^2*(SigmaPinj'+1i*SigmaQinj'));
end
	...
	for jj=1:length(Connected)
		...
		if any(volt_base(Bus1Ind(1))>volt_base(Bus2Ind(1)))
			...
			bus2BusInd=find(ismemberi(buslist,char(Bus2)));
					...
					if measFlag
			%Aggregate line power measurements
			updatedP=generation{Connected(jj),7}-generation{ii,7};
			updatedQ=generation{Connected(jj),8}-generation{ii,8};
			for iiii=1:3

				Meas{end+1,1}=3;
				Meas{end,2}=[buslist{ii} '.' num2str(iiii)];
				Meas{end,3}=[buslist{Connected(jj)} '.' num2str(iiii)];
				Meas{end,4}=real(updatedP(iiii,1));
				Meas{end,5}=imag(updatedP(iiii,1));
				Meas{end+1,1}=3;
				Meas{end,3}=[buslist{ii} '.' num2str(iiii)];
				Meas{end,2}=[buslist{Connected(jj)} '.' num2str(iiii)];
				Meas{end,4}=real(updatedP(iiii,2));
				Meas{end,5}=imag(updatedP(iiii,2));
				
				Meas{end+1,1}=4;
				Meas{end,2}=[buslist{ii} '.' num2str(iiii)];
				Meas{end,3}=[buslist{Connected(jj)} '.' num2str(iiii)];
				Meas{end,4}=real(updatedQ(iiii,1));
				Meas{end,5}=imag(updatedQ(iiii,1));
				Meas{end+1,1}=4;
				Meas{end,3}=[buslist{ii} '.' num2str(iiii)];
				Meas{end,2}=[buslist{Connected(jj)} '.' num2str(iiii)];
				Meas{end,4}=real(updatedQ(iiii,2));
				Meas{end,5}=imag(updatedQ(iiii,2));
...
tic
fprintf('\nRe-writing Measurements: ')
if measFlag
	%first delete all measurements we don't want.
	Meas(find([Meas{:,1}]==1),:)=[];
	Meas(find([Meas{:,1}]==2),:)=[];
	
	Vind=find([Meas{:,1}]==5);
	KillInds=Vind(find(~ismemberi(Meas(Vind,2),Ycomb)));
	Meas(KillInds,:)=[];
		
	%write new line measurements
	colOne=[ones(length(Sinj_new),1); 2*ones(length(Sinj_new),1)];
	colTwo=[Ycomb; Ycomb];
	colThree=cell(2*length(Ycomb),1); colThree(:) = {'sourcebus.1'};
	colFour=[real(Sinj_new);imag(Sinj_new)];
	colFive=[real(Sigma_new); imag(Sigma_new)];
	Mat=[num2cell(colOne), colTwo, colThree, num2cell(colFour), num2cell(colFive)];
	
	Meas=[MeasSrc;Meas;Mat];
	
	Meas(find([Meas{:,4}]==0 & [Meas{:,5}]==0),:)=[];
	Meas(find(isnan([Meas{:,4}])),:)=[];
end
...
if isfield(circuit_orig,'buslist')
