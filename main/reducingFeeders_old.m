...
for ii=1:length(generation(:,1))
	...
	for rr=1:length(r_jx)
		...
		if ~Flag(3) || ~isfield(circuit,'capacitor')
		...
		circuit.capacitor(end).name=['addedShunt_' char(Ycomb(Bus1Ind(rr)))];
		circuit.capacitor(end).phases=1;
		circuit.capacitor(end).bus1=char(Ycomb(Bus1Ind(rr)));
		circuit.capacitor(end).kv=VreducedVectAbs(Bus1Ind(rr))/sqrt(3);
		if real(r_jx(rr))<0
			circuit.capacitor(end).R=0;
		end
		w=2*pi*60;
		circuit.capacitor(end).R=real(r_jx(rr));
...
% %
% %
% % Yfill=zeros(length(buslist)*3); %create empty matrix to represtn all 3 PhaseInd connecitons
% % PhaseInd=reshape(repmat([1 2 3],length(buslist),1)',1,[])';
% % bus=reshape(repmat(buslist,1,3)',1,[])';
% % YfillOrder=strcat(strcat(bus,'.'),num2str(PhaseInd)); % createOrder vect
...
% % keepRows=ShuntRows(find(~ismemberi(ShuntRows,Source)));
% %
...
if Flag(1)
	...
	for loadModel=1:size(S_LD_new,2)
		...
		for ii=1:length(WriteLoads)
			...
			circuit.load(count).kV= VreducedVectAbs(WriteLoads(ii))/sqrt(3);
...
if Flag(2)
	...
	for ii=1:length(WritePVs)
		...
		% 		MatchInd=find(ismemberi(trfDownInd,[cell2mat(generation(busNum,4)); busNum]));
		circuit.pvsystem(count).kV= VreducedVectAbs(WritePVs(ii))/sqrt(3);
