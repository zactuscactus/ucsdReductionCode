function [ data ] = dssSimulation_TS_control( circuit, mode, t,V_predict, nodeOrder)
...
% 	LDpwr.kw = zeros(nt,length(circuit.load));
% 	LDpwr.kvar = zeros(nt,length(circuit.load));
% end
...
buses=buses([2,4]);
Bus1(ii)=find(ismemberi(nodeOrder,buses(1)));
Bus2(ii)=find(ismemberi(nodeOrder,buses(2)));

%change inverter
curveInd(ii)=find(ismemberi(curveNames,circuit.invcontrol(ii).vvc_curve1));
end
...
% 		buses=regexp(circuit.invcontrol(ii).name,'_','split');
% 		buses=buses([2,4]);
% 		dv(ii)=V_predict(find(ismemberi(nodeOrder,buses(1))),i)-V_predict(find(ismemberi(nodeOrder,buses(2))),i);
% 		
% 	%change inverter
% 	curveInd2=find(ismemberi(curveNames,circuit.invcontrol(ii).vvc_curve1));
% 	
% 	NewArr=circuit.xycurve(curveInd2).xarray+dv(ii);
% 	allOneString = sprintf('%.04f,' ,NewArr);
% 	allOneString = allOneString(1:end-1);% strip final comma
% 	dssText.command=['XYcurve.' char(curveNames(curveInd2)) '.Xarray=(' allOneString ')'];
% 	end
% 	toc
% 	tic
	for ii=1:length(circuit.invcontrol)
		dv(ii)=V_predict(Bus1(ii),i)-V_predict(Bus2(ii),i);
		

	NewArr=circuit.xycurve(curveInd(ii)).xarray+dv(ii);
	allOneString = sprintf('%.04f,' ,NewArr);
	allOneString = allOneString(1:end-1);% strip final comma
	dssText.command=['XYcurve.' char(curveNames(curveInd(ii))) '.Xarray=(' allOneString ')'];
	end
% 	toc
...
% % 		if isfield(circuit, 'load')
% % 			pw = dssgetval(o,circuit.load,'powers');
% % 			pw(find(isnan(pw)))=0;
% % 			LDpwr.kw(i,:) = -sum(pw(:,1:2:end),2)';
% % 			LDpwr.kvar(i,:) = -sum(pw(:,2:2:end),2)';
% % 		end
% % 		if i==100
% % 			killing=1;
% % 		end
